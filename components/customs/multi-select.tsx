...
export function MultiSelect(props: MultiSelectProps) {
    ...
    const handleUnselect = (option: MultiSelectOption) => {
        console.log("On Unselect", option, selected)
        handleOnChange((selected.filter(s => {
            console.log("On unselect: ", s, option);
    ...
    const selectables = options.filter(option => !selected.includes(option.label));

    return (
        <>
            {selected.length > 0 && <p className={"mb-1 font-light text-sm"}>{heading}</p>}
                                    ...
                                    return (
                                        <CommandItem
                                            ...
                                            onSelect={(value) => {
                                                ...
                                                setInputValue("")
                                                const newValue = selectables.find((option) => option.value === value);
                                                if(newValue)
