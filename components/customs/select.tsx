...
import { FilterTooltip } from "@/app/pathogen/arbovirus/dashboard/filters";
...
export function Select(props: SelectProps) {
  ...
  const { heading, selected, handleOnChange, optionToLabelMap, singleSelect } =
    props;
        ...
        <div className="group border border-b-2 border-background px-3 py-2 text-sm ring-offset-background rounded-md focus-within:ring-1 focus-within:ring-ring">
          <div className="flex flex-col gap-1 flex-wrap">
            ...
            <div className="flex">
              ...
              />
              <FilterTooltip
                className={cx("pl-2 -mr-2", props.tooltipContent ? "flex" : "hidden")}
            ...
            <div className="flex gap-1 flex-wrap">
              {createSelectOptionList(selected, optionToLabelMap).map(
                (selectedOption) => {
                  return (
                    <Badge
                      className={cn(
                        "rounded-sm hover:bg-backgroundHover p-1 bg-background"
                      )}
                    ...
                    >
                      ...
                      >
                        <X
                          className={cn(
                            "h-3 w-3 p-0.5 rounded-sm text-foreground hover:bg-background"
                          )}
                        />
                      </button>
                    </Badge>
