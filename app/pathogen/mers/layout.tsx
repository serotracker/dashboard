...
export default async function MersLayout({
  ...
  await queryClient.prefetchQuery({
    ...
    queryFn: () => request(process.env.NEXT_PUBLIC_PREVIEW_API_GRAPHQL_URL ?? '', faoMersEventFilterOptions)
  });
  ...
  await queryClient.prefetchQuery({
    ...
    queryFn: () => request(process.env.NEXT_PUBLIC_PREVIEW_API_GRAPHQL_URL ?? '', yearlyFaoCamelPopulationDataPartitionKeys)
  });
