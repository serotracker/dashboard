...
import Translate, {
  TranslateDate,
  getLanguageType,
  translateAntibodyTargets,
} from "@/utils/translate-util/translate-service";
...
function riskTag(riskLevel: string) {
  ...
  return (
    <div
      className={
        "text-center " + (popupColorStyles[riskLevel] ?? "bg-gray-200")
      }
    >
 ...
 */
function getTranslatedPopulationGroup(
  popGroupOptions: Record<string, string>[],
  englishPopGroup: string
) {
  let result = englishPopGroup;
  const popGroupWithTranslations = popGroupOptions.find(
    (x) => x.english === englishPopGroup
  );
...
interface SarsCov2StudyPopupContentProps {
  record: any;
}

export function SarsCov2StudyPopupContent({
  record,
}: SarsCov2StudyPopupContentProps) {
  return (
    <div className="w-[460px] bg-white pt-2">
      ...
      <div className={"flex justify-between bg-gray-200 w-full py-2 px-4"}>
        <div className={"w-1/3 font-semibold"}>
          ...
          <b>
            {" "}
            {record.serum_pos_prevalence
              ? `${(record.serum_pos_prevalence * 100).toFixed(1)}%`
              : "N/A"}
          </b>
        </div>
        <div className={"w-2/3"}>
          {record.sampling_start_date && record.sampling_end_date && (
            <>
              {`${TranslateDate(record.sampling_start_date)} ${Translate(
                "DateRangeTo"
              )} ${TranslateDate(record.sampling_end_date)}`}
            </>
          )}
        </div>
      </div>
      ...
      <div className={"py-2 px-4 max-h-[250px] overflow-auto"}>
        ...
        {row(
          ...
          record.denominator_value
            ?.toString()
            .replace(/\B(?=(\d{3})+(?!\d))/g, Translate(","))
        )}
        {row(
          Translate("AntibodyTarget"),
          record.antibody_target && record.antibody_target.length > 0
            ? translateAntibodyTargets(record.antibody_target)
            : "N/A"
        )}
        {row(
          Translate("PositiveCases"),
          record.cases_per_hundred
            ? record.cases_per_hundred.toFixed(1) + Translate("Per100")
            : "N/A"
        )}
        {row(
          Translate("Vaccinations"),
          record.full_vaccinations_per_hundred
            ? record.full_vaccinations_per_hundred.toFixed(1) +
                Translate("Per100")
            : "N/A"
        )}
      </div>
